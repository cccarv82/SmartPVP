    ...
    elseif PSC_MultiKillCount > 1 and PSC_DB.EnableMultiKillSounds then
        ...
        if soundPack == "LoL" then
            if PSC_MultiKillCount == 2 then
                soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\double_kill.mp3"
            elseif PSC_MultiKillCount == 3 then
                soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\triple_kill.mp3"
            elseif PSC_MultiKillCount == 4 then
                soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\quadra_kill.mp3"
            elseif PSC_MultiKillCount == 5 then
                soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\penta_kill.mp3"
            elseif PSC_MultiKillCount == 6 then
                soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\hexa-kill.mp3"
            elseif PSC_MultiKillCount == 7 then
                soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\legendary-kill.mp3"
            end
        else -- UT sounds
            local utSounds = {}
            if PSC_MultiKillCount == 2 then
...
function PSC_RegisterPlayerKill(playerName, killerName, killerGUID)
    ...
    if PSC_IsGrayLevelKill(playerLevel, level) then
        ...
        if PSC_Debug then
            print("[SmartPVP]: Gray kill registered against " .. playerName .. " (Level " .. level .. ")")
        end
    end
...
function PSC_RegisterNPCKill(npcName, npcID)
    ...
    if PSC_Debug then
        print("[SmartPVP]: Recorded kill for NPC: " .. npcName .. " (ID: " .. npcID .. ")")
    end
