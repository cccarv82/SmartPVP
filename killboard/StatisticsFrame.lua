...
-- CoA tem classes CUSTOM (Felsworn/Starcaller/...), nao as 9 vanilla. A tela guarda
-- a classe como nome de exibicao (ex: "Starcaller"), entao a legenda/agregacao usa
-- a mesma lista custom (senao nunca casa -> grafico vazio + legenda errada).
local ALL_CLASSES = (SmartPVP_AllClassNames and SmartPVP_AllClassNames())
    or {"Warrior", "Paladin", "Hunter", "Rogue", "Priest", "Shaman", "Mage", "Warlock", "Druid"}
...
local function getClassColor(class)
    ...
    -- CoA custom classes: resolve pelo mapa central (nome/token custom -> cor).
    -- SmartPVP_ClassInfo tambem cobre as classes base vanilla, entao unifica tudo.
    if SmartPVP_ClassInfo then
        local _, r, g, b = SmartPVP_ClassInfo(class)
...
local function PSC_PopulateSummaryStatsContainer(container, stats, isLocalPlayer, extraData, playerName)
    ...
    if stats.mostKilledPlayer and (stats.mostKilledCount or 0) > 0 and stats.mostKilledPlayer ~= "None" then
        local mkDisplay = (SmartPVP_ShortName and SmartPVP_ShortName(stats.mostKilledPlayer)) or stats.mostKilledPlayer
        local mostKilledText = mkDisplay .. " (" .. (stats.mostKilledCount or 0) .. ")"
...
function PSC_CreateStatisticsFrame()
    -- SmartPVP: esconde qualquer frame antigo com esse nome antes de criar outro.
    -- (Esta funcao RECRIA o frame a cada open; o objeto antigo, de mesmo nome,
    -- ficaria orfao flutuando na tela — a duplicata que aparecia no hub.)
    local _existing = _G["PSC_StatisticsFrame"]
    if _existing and _existing.Hide then _existing:Hide() end
