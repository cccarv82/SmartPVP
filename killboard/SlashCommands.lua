...
function PSC_SlashCommandHandler(msg)
    ...
    elseif command == "sendstats" then
        ...
        if not sent then
            PSC_Print("[SmartPVP]: " .. (reason or "Unable to send statistics."))
            return
        end

        PSC_Print("[SmartPVP]: Sent stats to " .. normalized .. ". They will only be able to receive them if they have addon version 4.2 or higher.")
