...
local function D(...)
    if PSC_Debug then
        print("|cFFFFD700[SmartPVP Network]|r", ...)
    end
end
...
local function DeserializeDetailedStats(payload)
    ...
    if not string.find(payload, "EOM:1") then
        if PSC_Debug then
            print("|cFFFFD700[SmartPVP Network]|r Incomplete payload received (EOM missing). Discarding.")
        end
        return nil
    end
...
function Network:SendCommMessageWithDebug(prefix, payload, channel, target, priority)
    if PSC_Debug then
        ...
        print("|cFFFFD700[SmartPVP Network]|r Sending message to ", channel, "(size:", #payload, "bytes):", msgPreview)
    end
...
function Network:BuildDetailedStats()
    if self.lastPlayerStats then
        if PSC_Debug then
            ...
            print("|cFFFFD700[SmartPVP Network]|r Using cached player stats.")
        end
...
function Network:GetBroadcastChannels()
    ...
    -- 3.3.5a: "INSTANCE_CHAT" nao existe (Cata+); em BG o canal de addon msg e
    -- "BATTLEGROUND". Alcanca a raid do BG, que e o que INSTANCE_CHAT faria.
    if PSC_CurrentlyInBattleground then
        table.insert(distributionList, "BATTLEGROUND")
    ...
    -- ponytail: 3.3.5 NAO tem canal de proximidade p/ addon msg ("YELL"/"SAY" so
    -- viraram validos no Cata+) -> "Unknown addon chat type". Removido. Sync agora
    -- so via RAID/PARTY/BATTLEGROUND/GUILD. Sem broadcast de proximidade solto.
...
function Network:BroadcastStats(providedStats)
    ...
    if PSC_InCombat then
        ...
        if PSC_Debug then
            print("|cFFFFD700[SmartPVP Network]|r In combat - deferring broadcast until out of combat.")
        end
        return
    end
    ...
    if self.lastBroadcastTime and (now - self.lastBroadcastTime < self.MIN_BROADCAST_INTERVAL) then
        ...
        if not self.deferredBroadcastTimer then
            ...
            if PSC_Debug then
                print("|cFFFFD700[SmartPVP Network]|r Too many updates. Deferring broadcast by " .. string.format("%.1f", delay) .. "s")
            end
    ...
    if self:IsThrottled() then
        if PSC_Debug then
            print("|cFFFFD700[SmartPVP Network]|r Network throttled (BULK queue full), deferring broadcast...")
        end
    ...
    if PSC_Debug then
        ...
        print(string.format("|cFFFFD700[SmartPVP Network]|r Stats Size: %dB x %d chans. Interval: %.1fs",
...
function Network:ValidateLeaderboardCache()
    ...
    if fixedCount > 0 and PSC_Debug then
        print("|cFFFFD700[SmartPVP Network]|r Fixed " .. fixedCount .. " corrupted cache entries.")
    end
end
...
function Network:OnCommReceived(prefix, message, distribution, sender)
    ...
    if PSC_Debug then
        print("|cFFFFD700[SmartPVP RX]|r From:", sender, "Len:", #message)
        print(message)
    end
    ...
    if msgType == "FULL" then
        ...
        if distribution == "WHISPER" then
            PSC_Print("[SmartPVP]: " .. uniqueName .. " sent you their stats, open the Leaderboard to view them.")
        end
