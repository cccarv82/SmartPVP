...
local function GetPlayerInfoFromUnit(unit)
    ...
    if UnitIsPlayer(unit) then
        ...
        -- SmartPVP: usa o TOKEN da classe (2o retorno) e resolve p/ o nome custom
        -- do server (CoA: Felsworn, Cultist, etc). Corrige nomes multi-palavra.
        local _loc, _token = UnitClass(unit)
        class = SmartPVP_ClassInfo(_token or _loc)
...
local function ProcessKillKeyMigrationSlice()
    ...
    while processed < KILL_KEY_MIGRATION_BUDGET do
        if killMigrationState.charIndex > #killMigrationState.charKeys then
            ...
            print("[SmartPVP]: Migration step 1 complete. Updated " .. killMigrationState.count .. " database entries.")
            killMigrationState = nil
            return
        end
...
function PSC_MigrateKillKeys()
    ...
    print("[SmartPVP]: Performing database update, this will cause your game to stutter for a few seconds...")
...
local function ProcessLossKeyMigrationSlice()
    ...
    while processed < LOSS_KEY_MIGRATION_BUDGET do
        if lossMigrationState.charIndex > #lossMigrationState.charKeys then
            ...
            print("[SmartPVP]: Death history migration complete. Normalized " .. lossMigrationState.count .. " database entries.")
            lossMigrationState = nil
            return
        end
...
function PSC_MigrateLossKeys()
    ...
    print("[SmartPVP]: Migrating death history to normalize player names...")
    C_Timer.After(0, ProcessLossKeyMigrationSlice)
end
...
function PSC_MigrateArenaZones()
    ...
    if migratedKills > 0 or migratedDeaths > 0 then
        print(string.format("[SmartPVP]: Arena zone migration complete. Fixed %d kill location(s) and %d death record(s).",
...
function PSC_MigratePlayerInfoCache()
    if not PSC_DB.PlayerInfoCacheMigrated then
        print("[SmartPVP]: Migrating player cache to support cross-realm players...")
        ...
        print("[SmartPVP]: Player cache migration complete!")
    end
end
...
function PSC_MigratePlayerInfoToEnglish(force)
    if not PSC_DB.PlayerInfoEnglishMigrated or force then
        ...
        print("[SmartPVP]: Data migration to English complete!")
    end
end
...
function ResetAllStatsToDefault()
    ...
    print("[SmartPVP]: All statistics have been reset!")
end
