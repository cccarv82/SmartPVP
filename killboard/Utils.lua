...
local function PSC_RunTaskQueue(taskQueue, onDone)
    ...
    local function runNextTask()
        ...
        if not success then
            print("[SmartPVP] Error in incremental calculation task " .. currentTask .. ": " .. tostring(result))
            result = true
        end
...
function PSC_ShowWhatsNewPopup(titleText, messageText, onCloseCallback, linkUrl)
    ...
    icon:SetTexture("Interface\\AddOns\\SmartPVP\\killboard\\img\\RedridgePoliceLogo.blp")
