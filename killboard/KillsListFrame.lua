...
local function FormatPlayerNameForDisplay(playerName)
    -- CoA e single-realm: o slug "-Rexxar" e o display "Conquest of Azeroth" sao o
    -- MESMO server, mas PSC_RealmName (=display) nunca casa com o slug -> tudo virava
    -- "Nome-Rexxar*". O sufixo de realm e sempre redundante aqui -> exibe so o nome.
    local name = playerName and playerName:match("^(.+)%-.+$")
    return name or playerName, playerName -- (display sem realm; fullname p/ tooltip)
end
