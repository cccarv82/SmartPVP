...
local function PSC_RunMigrationTaskQueue(taskQueue, onDone)
    ...
    local function runNextTask()
        ...
        if not success then
            print("[SmartPVP] Error in migration task " .. currentTask .. ": " .. tostring(result))
            result = true
        end
...
function PSC_PerformDataMigration(runSync)
    ...
    print("[SmartPVP]: Starting data migration...")
    ...
    print(string.format("[SmartPVP]: Imported %d player info cache entries.", infoImported))
    ...
    local function finalizeMigration()
        if PSC_Debug and killState then
            print(string.format("[SmartPVP]: Imported %d kills (%d kill keys normalized).", killState.totalImportedKills or 0, killState.counters.normalizedKeys or 0))
        end
        if PSC_Debug and lossState then
            print(string.format("[SmartPVP]: Imported %d deaths (%d loss keys normalized, %d assister names normalized).", lossState.counters.importedDeaths or 0, lossState.counters.normalizedKeys or 0, lossState.counters.normalizedAssisters or 0))
        end
        ...
        print("[SmartPVP]: Data migration complete!")
        print("[SmartPVP]: Reloading UI to finalize changes...")
...
function PSC_CheckForDataMigration()
    ...
    StaticPopupDialogs["PSC_IMPORT_CONFIRM"] = {
        ...
        OnCancel = function()
            PSC_DB_IMPORT = nil
            print("[SmartPVP]: Import cancelled. Data discarded.")
        end,
        timeout = 0,
        whileDead = true,
        hideOnEscape = true,
