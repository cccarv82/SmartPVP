...
function HandlePlayerDeath()
    ...
    if PSC_DB.EnableDeathSounds then
        ...
        if soundPack == "LoL" then
            ...
            soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\LoL\\" .. lolDeathSounds[randomIndex]
        else
            soundFile = "Interface\\AddOns\\SmartPVP\\killboard\\sounds\\UT\\you-have-failed-to-proceed.mp3"
        end
        PlaySoundFile(soundFile, "Master")
    end

    print("[SmartPVP]: You died, kill streak reset.")
...
function PSC_IsValidTarget(destFlags, destGUID)
    if destFlags and bit.band(destFlags, COMBATLOG_OBJECT_TYPE_PLAYER) > 0 and
       bit.band(destFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) > 0 then
        return true
    end

    -- CoA: o combat log nativo devolve flags 0/nao-confiaveis, entao o teste de
    -- flags acima falha p/ players. Se o GUID e de player, trata como alvo valido
    -- (voce so recebe PARTY_KILL de inimigo que voce matou; friendly nao da KB).
    if destGUID and SmartPVP_IsPlayerGUID(destGUID) then
        return true
    end
...
local function HandlePartyKillEvent(sourceGUID, sourceName, destGUID, destName)
    ...
    -- CoA: o GUID do combat log pode nao bater string-a-string com UnitGUID.
    -- Casa tambem por nome do char pra confirmar "meu killing blow".
    local isMyKB = (sourceGUID == PSC_PlayerGUID) or (sourceName and sourceName == PSC_CharacterName)
    ...
    if countKill then
        ...
        if npcID and PSC_TrackedNPCs[npcID] then
            ...
            if SmartPVP_IsPlayerGUID(destGUID) then
    ...
    if petDamage and (GetTime() - petDamage.timestamp) <= PET_DAMAGE_WINDOW then
        ...
        if countKill then
            ...
            if (npcID and PSC_TrackedNPCs[npcID] and petDamage.ownerGUID == PSC_PlayerGUID) or
               (npcID == nil and SmartPVP_IsPlayerGUID(destGUID)) then
...
local function HandlePlayerEnteringWorld()
    ...
    -- Self-heal: se PlayerKillCounts/PlayerInfoCache vierem nil (ex: wipe antigo
    -- bugado deixou nil), reconstroi -> senao NetworkHandler/Statistics crasham ao
    -- indexar PlayerKillCounts. ResetAllStatsToDefault refaz a estrutura completa.
    if not PSC_DB.PlayerKillCounts then
        ResetAllStatsToDefault()
    end
    PSC_DB.PlayerKillCounts = PSC_DB.PlayerKillCounts or { Characters = {} }
    ...
    if PSC_Debug then
        print("[SmartPVP]: Debug mode enabled.")
    end

    if not addonWelcomeMessageShown then
        addonWelcomeMessageShown = true
        print("[SmartPVP]: Click the minimap button or type /psc to use the addon.")
    end
...
function PSC_CheckBattlegroundStatus()
    if PSC_DB.ForceBattlegroundMode then
        if not PSC_lastInBattlegroundValue then
            print("[SmartPVP]: Forced battleground mode enabled.")
        end
    ...
    for _, bgMapId in ipairs(battlegroundZoneIds) do
        if (currentMapId == bgMapId) then
            if not PSC_lastInBattlegroundValue then
                local msg = "[SmartPVP]: Entered battleground. "
    ...
    if PSC_lastInBattlegroundValue then
        print("[SmartPVP]: Left battleground.")
    end
    PSC_lastInBattlegroundValue = false
    PSC_CurrentlyInBattleground = false
end
